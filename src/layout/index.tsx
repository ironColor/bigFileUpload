...
export const Layout:FC<{ keyConfig?: string }> = memo(({ keyConfig }) => {
  ...
  const svgTextRef = useRef<SVGRectElement>(null);
  ...
  useEffect(() => {
    window.addEventListener('scroll', (e) => {
      ...
      if (scrolled <= 0.1) {
        //@ts-ignore
        h1Ref.current?.style.opacity = (0.1 - scrolled) / 0.1
        //@ts-ignore
        h1Ref.current?.style.marginTop = scrolled * 1000 * -1 + 'px'
      } else {
        //@ts-ignore
      ...
      if (scrolled <= 0.2) {
        //@ts-ignore
        svgRef.current?.style.opacity = (scrolled - 0.1) / 0.1
      } else {
        //@ts-ignore
      ...
      if (scrolled >= 0.5) {
        //@ts-ignore
        svgTextRef.current?.style.opacity = (1 - scrolled) / 0.5
        tipRef.current?.classList.add('transparent')
      } else {
        //@ts-ignore
        svgTextRef.current?.style.opacity = 1
        tipRef.current?.classList.remove('transparent')
      }

        
      if (scrolled >= 0.95) {
        //@ts-ignore
        tipRef.current?.style.opacity = (1 - scrolled) / 0.05
      } else {
        //@ts-ignore
